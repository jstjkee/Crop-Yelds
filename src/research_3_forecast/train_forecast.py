...
from src.research_3_forecast.build_dataset import (
    describe_research_3_dataset,
...
def train_one_model(
    ...
    return build_metrics_row(
        ...
        feature_mode=feature_mode,
        split_name=f"research_3_{split_name}_test",
...
def main(
    ...
    selected_splits = list(cfg.get("splits", ["year"]))
    selected_feature_sets = list(cfg.get("feature_sets", ["full"]))
    selected_seeds = _normalize_seeds(seed=seed, seed_list=seed_list)

    print(f"[research_3] selected seeds: {selected_seeds}")

    info = describe_research_3_dataset()
    ...
    for current_seed in selected_seeds:
        ...
        for split_name in selected_splits:
            for feature_set_name in selected_feature_sets:
                prepared = prepare_forecast_dataset(
                    split_name=split_name,
                    feature_set_name=feature_set_name,
                )
                ...
                joblib.dump(
                    ...
                    cfg["results"]["models"]
                    / f"research_3_preprocessor_{split_name}_{feature_set_name}.joblib",
                )

                with open(
                    cfg["results"]["models"]
                    / f"research_3_crop_mapping_{split_name}_{feature_set_name}.json",
                    "w",
                    encoding="utf-8",
                ) as f:
