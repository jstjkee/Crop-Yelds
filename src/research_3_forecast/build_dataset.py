...
def apply_weather_month_cutoff(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if cutoff is None:
        return df.drop(columns=weather_cols, errors="ignore")
    ...
    print(f"[research_3] weather_month_cutoff={cutoff} | dropped weather cols={len(cols_to_drop)}")
...
def describe_research_3_dataset() -> dict[str, Any]:
    cfg = RESEARCH_3_CONFIG
    df = load_enriched_dataframe()
    df = clean_enriched_dataframe(df)
